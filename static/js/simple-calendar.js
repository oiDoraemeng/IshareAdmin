...
var myConfig = {};
window._clickSP = function (obj) {
    var $obj = $(obj), status = $obj.attr("data-key");

    var unClicked = $obj.hasClass("wx-un-clicked");
    if (unClicked) {
        $obj.removeClass("wx-un-clicked");
        $(".sc-days").find(".selected_" + status).removeClass("none").show();
    } else {
        $obj.addClass("wx-un-clicked");
        $(".sc-days").find(".selected_" + status).addClass("none").hide();
    }
}
window._MonthWeekToDay = 42;//1个月最多显示多少天
...
var SimpleCalendar = function () {
    ...
    _createClass(SimpleCalendar, [{
        ...
        value: function create() {
            ...
            //actions
            header.innerHTML += '<div class="sc-actions" style="min-width: 230px !important;">'
            ...
            header.innerHTML = header.innerHTML + '<div class="sc-actions"><span class="sc-return-today ">返回今天</span><span class="sc-time"></span></div>';
            header.innerHTML = header.innerHTML + '<div class="sc-actions sc-mark-sp" id="sc-mark-sp"></div>';
...
function _addMask(parentObj, val, status, statusText) {
    var color = myConfig.markColor[status];
    if (typeof (val) == 'object') {
        // console.log(val)
        var title = val.title, name = val.name, ratio = val.ratio;
        if (title) title = title.trim();
        else title = '';
        $(parentObj).find('.wx-mark').append(
            "<li onclick='toDetails(\""+val.date+"\")' class='wx-mark-li selected_" + status + "' style='border-left: 3px solid " + color + ";'> "
            + "<p>" + title + "</p><p><a href='/calendar_search/"+val.date+"'>" + name + " | " + ratio + "</a></p>" + "</li>");

        if (!myConfig.sp[status]) {
            $("#sc-mark-sp").append("<span class='wx-sp-a' onclick='window._clickSP(this)' data-key='" + status + "'>" +
                "<span class='layui-badge-dot' style='background-color: " + color + ";'></span>" + statusText + "</span>");
            myConfig.sp[status] = color;
        }
    }
}

function toDetails(date){
    console.log(date)
    window.location.href='/calendar_search/'+date
}
