;!function (win) {
    ...
    let cur_index = md5("/welcome?token="+sessionStorage.getItem('token'))
    ...
    Xadmin.prototype.init = function () {
        ...
        if (sessionStorage.getItem('cur_index')) {
            cur_index = sessionStorage.getItem('cur_index')
        }

        element.tabChange('xbs_tab', cur_index);
    };
    /**
     * [end 执行结束要做的]
    ...
    Xadmin.prototype.end = function () {
        ...
        let cur_url = sessionStorage.getItem('cur_url')

        $('.left-nav').find('a').removeClass('active');
        $("[my-url='" + cur_url + "']").addClass('active');
    ...
    Xadmin.prototype.add_tab = function (title, url, is_refresh) {
        ...
        //重复点击
        sessionStorage.setItem('cur_index', id)
        sessionStorage.setItem('cur_url',url)
    ...
    Xadmin.prototype.get_data = function () {
        if (typeof is_remember != "undefined")
            return false;
        return layui.sessionData('tab_list')
    }
    /**
     * [set_data 增加某一项]
    ...
    Xadmin.prototype.del_data = function (id) {
        ...
        if (typeof id != "undefined") {
            ...
            layui.sessionData('tab_list', null);
        }
    };
    /**
     * [del_other_data 删除其它]
...
layui.use(['layer', 'element', 'jquery'], function () {
    ...
    $(".layui-tab-title").on('click', 'li', function (event) {
        ...
        var tablist = layui.sessionData('tab_list')
        var url = tablist[this_index].url
        // console.log(url)
        sessionStorage.setItem('cur_index', this_index)
        sessionStorage.setItem('cur_url',url)

        $('.left-nav').find('a').removeClass('active');
        $("[my-url='" + url + "']").addClass('active');
    });
